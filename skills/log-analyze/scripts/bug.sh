...
usage: bug.sh [text] [options]
  ...
  --priority P         state-folder backlogs only: stamp a `**Priority:** P` line
                       into the drop's header. P is A1, A2, or C — B is what a
                       missing line means, so omit the flag for B. Priority only
                       reorders the pipeline; every ticket still stops at Tom's
                       gate. (error in legacy flat backlogs)
...
STATUS="open"; SOURCE="manual"; SLUG_OVERRIDE=""; BACKLOG_DIR=""; PRIORITY=""
...
while [[ $# -gt 0 ]]; do
  opt="$1"
  case "$opt" in
    --title|--summary|--problem|--repro|--notes|--status|--source|--slug|--backlog-dir|--priority)
...
# Normalize + validate --priority early, before any writes. B is deliberately
# rejected: a missing **Priority:** line already means B everywhere in the
# pipeline, so stamping it would create two spellings of the same state.
if [[ -n "$PRIORITY" ]]; then
  PRIORITY="$(printf '%s' "$PRIORITY" | tr '[:lower:]' '[:upper:]')"
  case "$PRIORITY" in
    A1|A2|C) ;;
    B) echo "error: --priority B is the default — omit the flag" >&2; exit 2;;
    *) echo "error: --priority must be A1, A2, or C (got: $PRIORITY)" >&2; exit 2;;
  esac
fi
...
#   state-folder (maildir style, VinRaptor) - raw drops live in 0-inbox/,
#     backlog.md is GENERATED by bin/backlog-index. (The 0-inbox-express lane
#     was retired 2026-08-08; urgency is a **Priority:** header line now.)
...
if [[ "$MODE" == "state-folder" ]]; then
  STATE_DIR="$BACKLOG_DIR/0-inbox"
elif [[ -n "$PRIORITY" ]]; then
  echo "error: --priority requires a state-folder backlog" >&2; exit 2
fi
...
slug_taken() { # $1 = candidate slug -> exit 0 if already used
  local s="$1" dir
  if [[ "$MODE" == "state-folder" ]]; then
    for dir in 0-inbox 1-todo 2-initial-analysis-done 3-analyzed \
...
# 0-inbox/. backlog.md is generated, so we
...
if [[ "$MODE" == "state-folder" ]]; then
  BODY="$(
    ...
    # Own line at column 0: the pipeline's item_priority() and triage both
    # match /^\*\*Priority:\*\*/, so it must not share the Type line.
